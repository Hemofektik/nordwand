function MusicPlayer(songArray, sfxDict) {
	// Constants
	...
	this.sounds = sfxDict;
	this.sound_dir = "content/sounds/";
	this.music_dir = "content/music/";
	this.inited = false;
	...
	this.init = function ()
	{
	    ...
	    for (var i in this.sounds)
	    {
	        ...
	        this.sounds[i].push([new Audio(this.sound_dir + src), new Audio(this.sound_dir + src), new Audio(this.sound_dir + src), new Audio(this.sound_dir + src), new Audio(this.sound_dir + src)]);
	...
	this.load_song = function() {
		if (this.songs.length > this.current_song) {
            ...
            this.song_audio = new Audio(this.music_dir + this.songs[this.current_song][0]);
			...
			if (infobox && titlebox && artistbox) {
				SetTextOfElement( titlebox, this.songs[this.current_song][1] );
				SetTextOfElement( artistbox, this.songs[this.current_song][2] );
	...
	this.mute = function() {
		if (!this.muted) {
			...
			SetTextOfElement( document.getElementById("mute").children[0], "Unmute sounds [M]" );
		}
		else {
			// Unmute
			this.song_audio.play();
			...
			SetTextOfElement( document.getElementById("mute").children[0], "Mute sounds [M]");
		}
	};
